...
void input_board() {
    scanf(" %d", &size);
    mem_clear_size = size*u_char_size*20;
    mem_line_size  = size*u_char_size;
    int in, exp;
    for(i=0; i<size; i++) {
        for(j=0; j<size; j++) {
            scanf(" %d", &in);
...
int IsNotUsefull(int lv) {
    ...
    possible_mxbit = max_tmp + (FIND_LV - lv);
